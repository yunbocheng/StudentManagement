...
<%--      <div align="center">
      ...
      <div>
        <table class="table table-hover" style="width: 1100px;margin-top: 30px;font-size: 20px" align="center">
          ...
          <tbody align="center">
          <c:forEach items="${sessionScope.page.items}" var="stu">
            <tr>
              <td>
                  ${stu.num}
              </td>
              ...
              <td>
                <form action="${path }/Select" method="post" style="margin-bottom: 2px">
                  <input type="hidden" name="status" value="all">
                  <input type="hidden" name="num" value="${stu.num}">
                  <input type="submit" value="详细信息">
                </form>
              ...
              <td>
                <form action="${path }/Update" method="post" style="margin-bottom: 2px">
                  <input type="hidden" name="status" value="updateTurn">
                  <input type="hidden" name="room" value="${stu.room}">
                  <input type="hidden" name="num" value="${stu.num}">
                  <input type="submit" value="修改">
              ...
              <td style="padding-top:2px">
                <form action="${path }/delete" method="post" style="margin-bottom: 13px;padding-top: 8px">
                  <input type="hidden" name="status" value="deleteByNum">
                  <input type="hidden" name="num" value="${stu.num}">
                  <input type="hidden" name="room" value="${stu.room}">
                  <input type="submit" value="删除">
      ...
      <div id="page_nav">
        ...
        <a href="${path }/Select?status=page&id=1">首页</a>
        <c:choose>
          <c:when test="${sessionScope.page.pageNo eq 1}">
            上一页
          </c:when>
          <c:otherwise>
            <a href="${path }/Select?status=page&id=${sessionScope.page.pageNo-1}">上一页</a>
          </c:otherwise>
        </c:choose>
        【${sessionScope.page.pageNo}】
        <c:choose>
          <c:when test="${sessionScope.page.pageNo eq sessionScope.page.pageTotal}">
            下一页
          </c:when>
          <c:otherwise>
            <a href="${path }/Select?status=page&id=${sessionScope.page.pageNo+1}">下一页</a>
          </c:otherwise>
        </c:choose>
        <a href="${path }/Select?status=page&id=${sessionScope.page.pageTotal}">尾页</a>
